...
for (my $epoch_index = 0; $epoch_index < $epoch_count; $epoch_index++) {
  ...
  for (my $m_to_n_func_index = 0; $m_to_n_func_index < @$m_to_n_func_infos; $m_to_n_func_index++) {
    ...
    $adam_infos->[$m_to_n_func_index]{biase_moments} = $biase_moments;
    
    # Adamのバイアスに関するベロシティを0で初期化して作成
    my $baise_velocities = array_new_zero(scalar @$biases);
    $adam_infos->[$m_to_n_func_index]{baise_velocities} = $baise_velocities;
    ...
    $adam_infos->[$m_to_n_func_index]{weight_moments_mat} = $weight_moments_mat;
    ...
    $adam_infos->[$m_to_n_func_index]{weight_velocities_mat} = $weight_velocities_mat;
  }
