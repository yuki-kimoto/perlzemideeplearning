...
# Adamのパラメーター
my $adam_much_small_value = 1e-8;
my $adam_before_moment_weight = 0.9;
my $adam_before_velocity_weight = 0.999;
...
# 50個の中間出力を通って        (中間出力)
# 0～9の10個に分類する          (出力)
my $neurons_count_in_layers = [784, 30, 50, 10];
...
for (my $epoch_index = 0; $epoch_index < $epoch_count; $epoch_index++) {
  ...
  # Adamの情報
  my $adam_update_infos = [];
  
  # Adamのバイアスと重みに関するモーメントとベロシティの初期化
  for (my $m_to_n_func_index = 0; $m_to_n_func_index < @$m_to_n_func_infos; $m_to_n_func_index++) {
    my $m_to_n_func_info = $m_to_n_func_infos->[$m_to_n_func_index];
    my $biases = $m_to_n_func_info->{biases};
    my $weights_mat = $m_to_n_func_info->{weights_mat};
    
    # Adamのバイアスに関するモーメントを0で初期化して作成
    my $biase_moments = array_new_zero(scalar @$biases);
    $adam_update_infos->[$m_to_n_func_index]{biase_moments} = $biase_moments;
    
    # Adamのバイアスに関するベロシティを0で初期化して作成
    my $baise_velocities = array_new_zero(scalar @$biases);
    $adam_update_infos->[$m_to_n_func_index]{baise_velocities} = $baise_velocities;
    
    # Adamの重みに関するモーメントを0で初期化して作成
    my $weight_moments_mat = {
      rows_length => $weights_mat->{rows_length},
      columns_length => $weights_mat->{columns_length},
      values => array_new_zero(scalar @{$weights_mat->{values}}),
    };
    $adam_update_infos->[$m_to_n_func_index]{weight_moments_mat} = $weight_moments_mat;
    
    # Adamの重みに関するベロシティを0で初期化して作成
    my $weight_velocities_mat = {
      rows_length => $weights_mat->{rows_length},
      columns_length => $weights_mat->{columns_length},
      values => array_new_zero(scalar @{$weights_mat->{values}}),
    };
    $adam_update_infos->[$m_to_n_func_index]{weight_velocities_mat} = $weight_velocities_mat;
  }
...
sub backprop {
  ...
  for (my $m_to_n_func_index = 0; $m_to_n_func_index < @$m_to_n_func_infos; $m_to_n_func_index++) {
    ...
    # 活性化された出力 - 出力に活性化関数を適用 中間出力はrelu関数、最後の出力はsoftmax関数
    my $activate_outputs;
    if ($m_to_n_func_index == @$m_to_n_func_infos - 1) {
      $activate_outputs = softmax($outputs);
    }
    else {
      $activate_outputs = array_relu($outputs);
    }
  ...
  for (my $m_to_n_func_index = @$m_to_n_func_infos - 2; $m_to_n_func_index >= 0; $m_to_n_func_index--) {
    ...
    my $biase_grads = array_mul($mul_forword_weights_transpose_mat_forword_biase_grads_mat_values, $grads_outputs_to_array_relu);
